...
def get_odds_for_bet(home, away, cat, label, pred):
    """Intoarce cota bookmaker pentru un pariu specific (daca e disponibila)."""
    o = MARKET_ODDS.get((home, away), {})
    if not o: return None
    if cat == 'result':
    ...
    if cat == 'goals_ou':
        if 'Over 2.5' in label:  return o.get('o25')
        if 'Under 2.5' in label: return o.get('u25')
    if cat == 'gg':
...
def generate_bets_v3(home, away, wp_h, wp_d, wp_a, lh, la, mat, helo=1700, aelo=1700, ref_yc=1.0, ref_fouls=1.0):
    ...
    bets += [
        (f'Draw No Bet {home[:14]}', 'Victorie', round(dnb_h, 1), 'dnb'),
        (f'Draw No Bet {away[:14]}', 'Victorie', round(dnb_a, 1), 'dnb'),
    ]
